...
// From https://github.com/mdda/DelaunayScala/blob/master/src/main/scala/Delaunay.scala
object Delaunay {

  /**
   * Fastest version here.
   */
  def Delaunator(points: Iterable[Vec2]): Seq[(Int, Int, Int)] = {
    val d = kit.Delaunator.fromPoints(points)
    for (i <- d.triangles.indices by 3) yield (d.triangles(i), d.triangles(i+1), d.triangles(i+2))
  }
  ...
  */
  ...
  def Triangulation_n4(measurements : List[Vec2]) : Seq[(Int, Int, Int)] = {
